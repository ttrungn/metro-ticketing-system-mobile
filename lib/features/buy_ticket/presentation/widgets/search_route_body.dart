...
class SearchRouteBody extends StatelessWidget {
  final routeController = TextEditingController();
  final entryStationController = TextEditingController();
  final exitStationController = TextEditingController();

  SearchRouteBody({super.key});

  @override
  Widget build(BuildContext context) {


    return  BlocBuilder<SearchRouteCubit,SearchRouteState>(builder: (context, state)  {
      List<DropdownMenuEntry<dynamic>> routes = [];
      List<DropdownMenuEntry<dynamic>> stations = [];
      if(state.routes.isNotEmpty){
        routes = state.routes.map((e) => DropdownMenuEntry(value: e.id, label: e.name)).toList();
      }
